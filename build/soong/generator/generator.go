...
func init() {
	android.RegisterModuleType("aris_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = arisExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = arisExpandVariables(ctx, dep_file)
	...
	cmd := arisExpandVariables(ctx, String(g.properties.Cmd))
